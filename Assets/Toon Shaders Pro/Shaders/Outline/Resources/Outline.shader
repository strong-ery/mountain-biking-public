Shader "Hidden/ToonShadersPro/URP/Outlines"
{
    SubShader
    {
		...
		Pass
        {
			...
			// NEW: Add shader feature for depth normal layer masking
			#pragma shader_feature_local_fragment _USE_DEPTH_NORMAL_LAYER_MASK
			...
			// NEW: Depth normal layer mask texture
			#ifdef _USE_DEPTH_NORMAL_LAYER_MASK
			TEXTURE2D(_DepthNormalMask);
			#endif
            ...
            float4 frag (Varyings i) : SV_TARGET
            {
				...
				// NEW: Check if layer masking is enabled and if this pixel should be processed
				#ifdef _USE_DEPTH_NORMAL_LAYER_MASK
				float maskValue = SAMPLE_TEXTURE2D(_DepthNormalMask, sampler_LinearClamp, i.texcoord).r;
				// If layer masking is enabled and this pixel is not in a masked layer, skip outline detection
				if (maskValue < 0.0001f)
				{
					return col;
				}
				#endif
				...
				// NEW: For layer masking, we need to check if adjacent pixels are also in valid layers
				#ifdef _USE_DEPTH_NORMAL_LAYER_MASK
				float maskLeft = SAMPLE_TEXTURE2D(_DepthNormalMask, sampler_LinearClamp, leftUV).r;
				float maskRight = SAMPLE_TEXTURE2D(_DepthNormalMask, sampler_LinearClamp, rightUV).r;
				float maskBottom = SAMPLE_TEXTURE2D(_DepthNormalMask, sampler_LinearClamp, bottomUV).r;
				float maskTop = SAMPLE_TEXTURE2D(_DepthNormalMask, sampler_LinearClamp, topUV).r;
				
				// Only use adjacent pixels that are in valid layers, otherwise use center pixel values
				bool useLeft = maskLeft > 0.0001f;
				bool useRight = maskRight > 0.0001f;
				bool useBottom = maskBottom > 0.0001f;
				bool useTop = maskTop > 0.0001f;
				#endif
				...
				// NEW: Apply layer masking to color sampling
				#ifdef _USE_DEPTH_NORMAL_LAYER_MASK
				col0 = useLeft ? col0 : col.rgb;
				col1 = useRight ? col1 : col.rgb;
				col2 = useBottom ? col2 : col.rgb;
				col3 = useTop ? col3 : col.rgb;
				#endif
				...
				// NEW: Apply layer masking to depth sampling
				#ifdef _USE_DEPTH_NORMAL_LAYER_MASK
				float centerDepth = Linear01Depth(GetSceneDepth(i.texcoord), _ZBufferParams);
				depth0 = useLeft ? depth0 : centerDepth;
				depth1 = useRight ? depth1 : centerDepth;
				depth2 = useBottom ? depth2 : centerDepth;
				depth3 = useTop ? depth3 : centerDepth;
				#endif
				...
				// NEW: Apply layer masking to normal sampling
				#ifdef _USE_DEPTH_NORMAL_LAYER_MASK
				float3 centerNormal = DecodeNormal(SAMPLE_TEXTURE2D_X(_CameraNormalsTexture, sampler_LinearClamp, i.texcoord));
				normal0 = useLeft ? normal0 : centerNormal;
				normal1 = useRight ? normal1 : centerNormal;
				normal2 = useBottom ? normal2 : centerNormal;
				normal3 = useTop ? normal3 : centerNormal;
				#endif
