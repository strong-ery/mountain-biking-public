...
namespace ToonShadersPro.URP
{
    ...
    public class OutlineEditor : VolumeComponentEditor
    {
        ...
        // Depth Normal Layer Masking parameters
        SerializedDataParameter useDepthNormalLayerMasking;
        SerializedDataParameter depthNormalLayerMask;
        SerializedDataParameter invertDepthNormalLayerMask;
        SerializedDataParameter depthNormalLightModes;
        SerializedDataParameter depthNormalRenderQueue;
        SerializedDataParameter depthNormalMaskIgnoreDepth;
        ...
        public override void OnEnable()
        {
            ...
            // Initialize depth normal layer masking parameters
            useDepthNormalLayerMasking = Unpack(o.Find(x => x.useDepthNormalLayerMasking));
            depthNormalLayerMask = Unpack(o.Find(x => x.depthNormalLayerMask));
            invertDepthNormalLayerMask = Unpack(o.Find(x => x.invertDepthNormalLayerMask));
            depthNormalLightModes = Unpack(o.Find(x => x.depthNormalLightModes));
            depthNormalRenderQueue = Unpack(o.Find(x => x.depthNormalRenderQueue));
            depthNormalMaskIgnoreDepth = Unpack(o.Find(x => x.depthNormalMaskIgnoreDepth));
        ...
        public override void OnInspectorGUI()
        {
            ...
            if(activeOutlineType == OutlineType.DepthNormalOutlines)
            {
                ...
                // Add depth normal layer masking section
                GUILayout.Space(10);
                EditorGUILayout.LabelField("Depth Normal Layer Masking", headerStyle);
                PropertyField(useDepthNormalLayerMasking);
                
                if(useDepthNormalLayerMasking.value.boolValue)
                {
                    PropertyField(depthNormalLayerMask);
                    PropertyField(invertDepthNormalLayerMask);
                    PropertyField(depthNormalLightModes);
                    PropertyField(depthNormalRenderQueue);
                    PropertyField(depthNormalMaskIgnoreDepth);
                }
            }
